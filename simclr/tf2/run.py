...
def main(argv):
  ...
  with strategy.scope():
    ...
    # model = model_lib.Model(num_classes, cifar_stem=False, resnet_depth=18)
    # model = hub.KerasLayer("https://tfhub.dev/google/imagenet/resnet_v2_50/feature_vector/4", trainable=False)
  if FLAGS.mode == 'eval':
    ...
    with strategy.scope():
      ...
      ds = builder.as_dataset(split='train', shuffle_files=True, batch_size=FLAGS.train_batch_size, )
    ...
    with strategy.scope():
      ...
      def train_multiple_steps(iterator):
        ...
        for _ in tf.range(steps_per_loop):
          ...
          with tf.name_scope(''):
            batch = next(iterator)
            images, labels = tf.cast(batch["image"], float), tf.expand_dims(batch["label"], 0)
            features, labels = images, {'labels': labels}
            strategy.run(single_step, (features, labels))
